...
\medskip 
 ...
 Ke výměně stejného objemu dat dojde z důvodu použitého komunikačního protokolu i v opačném směru. Z RPi do rozšiřující jednotky přichází tři 11-bitové informace o šířce plnění PWM. Součástí přenosu jsou ještě 3 bity odemykající poloviční H-můstky a 3 bity určené k jejich úplné deaktivaci. Poslední využitý bit pak při změně z logické 0 do 1 povoluje resetování AD převodníku. Celkem je v tomto směru komunikace využito jen 40 ze 128 bitů.
...
V plošném spoji výkonového stupně {\em 3P-MOTOR-DRIVER-1} je proto integrován čtyřkanálový, 12-bitový AD převodník ADS7841. Převodník může pracovat buď ve 12-bitovém nebo jen v 8-bitovém režimu \cite[adc_man]. 
...
Na začátku cyklu, který je detekován při přetečením counteru signálem "sync", je tedy výstup PWM nastaven na 1. Zároveň je kontrolována podmínka "match=count", při jejím splnění dojde nastavení výstupu PWM na 0.

Protože potřebujeme generovat tři signály PWM, je nutné tyto bloky vytvořit práě tři. Counter je však vyvořen jen jeden a je všemi bloky sdílen.  
