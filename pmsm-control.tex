...
Tímto způsobem byla transformační matice rozšířenana velikost 3x3. Matice je pro $z\not=0$ a $k\not=0$ regulární, je tak možné vytvořit matici inverzní a celé inverzní zobrazení.
 ...
 Pomocí iverzní Clarkovy transformace je lze vypočítat jednotlivé statorové proudy. Z rovnic \ref[rce1] až \ref[clark_napet_rce_3] pak vyplývá, že proudy je možné řídit napětím na vinutí. 
 
 Nyní je tedy třeba  napětí $u_a$, $u_b$ a $u_c$, které představují akční zásahy pro řízení proudů, přepočítat na napětí $u_1$, $u_2$ a $u_3$, která jsou již přímo úměrná šířce plnění PWM. Pro výpočet je uvažována zapojení typu \uv{delta}. Schématicky problém postihuje obrázek \ref[delta_obr]. 
...
\eqalignno{
 ...
 & \eqmark \cr}
 $$
Parametr $t$ je možné volit libovolně. 
...
\begtt
    ...
    uc=ic;
\endtt
Nyní zbývá přepočítat fázová napětí a přiřadit jejich hodnoty šířce PWM:
\begtt
