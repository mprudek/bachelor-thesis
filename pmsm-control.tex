...
\medskip 
 ...
 kde $\hat{a}$ a $\hat{a}^2$ jsou operátory posouvající fáze o~$\delta=120^{\circ}={2\pi\over3}rad$. Operátory mají podobu: $ \hat{a}=e^{j\delta} $ a $\hat{a}^2=e^{2j\delta}$. Po jejich dosazení je možné psát:

 $$ 
 \overline{i} = i_{a}+i_{b}e^{j\delta}+i_{c}e^{2j\delta} \eqmark$$
  %Použijeme Eulerův vzorec $e^{j\phi}=\cos{\phi} + j\sin{\phi}$:
 ...
 Z~rovnice \ref[rce_cplx] je tak možné napsat maticové vyjádření Clarkovy transformace:
...
Složky napětí v {\em dqo} souřadném systému je možné přepsat jako:
