...
\medskip 
 ...
 Pro matematický popis dějů v PMS motorech az vláště pak pro \uv{Sensorless} řízení jsou vhodným nástrojem Clarkova a parkova tranformace. Jejich použití je uvedeno v této sekci.
 ...
 $$ \overline{i} = i_{sa}+\hat{a}i_{sb}+\hat{a}^2i_{sc} \eqmark$$
 kde $\hat{a}$ a $\hat{a}^2$ jsou operátory posouvajíci fázi o $120^{\circ}$, $ \hat{a}=e^{{2j\pi}/{3}} $ a $\hat{a}^2=e^{{4j\pi}/{3}}$.
...
Pro dosažení maximálních momentových účinků, je totiž nutné, aby vektor megnetické indukce, magnetického pole vyvolávaného proudy protékající vinutím statoru, svíral pravý úhel s vektorem mg. indukce mg. pole permanentního magnetu rotoru. Pro tyto účely je tedy vhodné veličiny fixovat právě k souřadné soustavě rotoru. 

Pro statickou soustavu jsme úspěchem využili Clarkovu transformaci, nyní, pro rotující vztažnou soustavu použijeme Parkovu (dq0) transformaci.
...
Z hardwarových důvodů není možné pomocí pwm vyvolat ve fázi záporný proud. Tato skutečnost ale není na závadu, protože soustava tří fází je přeurčená ($ i_{sa} + i_{sb} + i_{sc} =0 $). Z toho důvodu záporný proud vznikne jako jako opačná hodnota součtu proudů kladných. Pro pohyb ve směru hodinových ručiček může být ve zdrojovém kódu taková operace provedena následovně:
