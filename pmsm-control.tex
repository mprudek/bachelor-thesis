 ...
 The first part of thesis deals with theory of PMSM, including their construction, mathematical description and possibilities of control. After this takes place presentation of used hardware, Raspberry Pi, expansion unit and motor.
 
 Thesis continues with description of operating system GNU/Linux and its Real-Time modification.
 
 To fulfil communication between expansion board and superior system protocol SPI is used. Its implementation in FPGA and Raspberry Pi is described in next chapter.
 
 For the purpose of measure currents in stator windings is used solution which utilizes Hall effect. Voltages, that are obtained this way are then processed in A/D converter. To read values from A/D converter simple communication protocol is implemented in FPGA.    
...
Na trhu je dnes celá řada malých zařízení, která nedosahují kvalit potřebných pro průmyslové nasazení. Je ale možné je využít k návrhu řízení a komunikace s výkonovou částí. Takto vytvořená řešení jsou pak použitelná i při nasazení na profesionálnějších systémech.
 ...
 Pro propojení rozšiřující desky a nadřazeného systému (RPi) bylo třeba vybrat jednoduchý komunikační protokol. Požadavkem byla možnost obousměrné komunikace a dostatečná rychlost, která umožní pro  požadovanoufrekvenci řídicí smyčky přenést dostatečný objem dat. Bylo také nutné, aby použitý protokol byl pokud možno jednoduše realizovatelný v~FPGA obvodu a zároveň ho bylo možné pohodlně zpracovat v~jádře operačního systému na RPi. Více o výběru a implementaci komunikačního protokolu v sekci \ref[kom_prot_sec].
...
Tímto způsobem byla transformační matice rozšířenana velikost 3x3. Matice je regulární a je tak možné vytvořit matici inverzní a celé inverzní zobrazení.
 ...
 Minulá kapitola shrnula teorii týkající se konstrukce bezkartáčových motorů a jejich řízení. Následovatbude popis konkrétního technického vybavení použitého pro tuto práci. Budou tak popsány parametry použitého motoru, dále Raspberry Pi a jeho rozšiřujíci jednotky.  
...
Kde "pxmc_sin_fixed_inline" je funkce z~knihovny {\em PXMC}, která celočíselně, s~přesností 10 bitů, počítá hodnotu sinu. Proměnná "pos" udává úhlovou pozici rotoru. 
